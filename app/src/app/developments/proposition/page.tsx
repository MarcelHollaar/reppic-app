...
export default function PropositionPage() {
  ...
  const courses = [
    {
      ...
      tag: "PROPOSITION",
      title: "Proposition Tutorial: Device Frames and Scrolling",
      author: {
        name: "The Sales Studios",
        role: "UX Designer",
  ...
  return (
    <>
      <div className="">
        <button
          className="tw-flex tw-mb-6 tw-items-center tw-gap-2 tw-ml-3 !tw-bg-blue-100 tw-text-blue-900 tw-rounded-full tw-h-10 tw-px-5 tw-font-medium tw-text-[0.875rem] hover:tw-bg-blue-200"
          onClick={() => router.push("/developments")}
        >
          <ArrowLeftIcon className="tw-w-4 tw-h-4 tw-text-button tw-font-medium" />
          Development
        </button>
        <div className="tw-flex tw-flex-row tw-justify-between ">
          <h3 className="tw-ml-4 tw-text-xl tw-font-semibold tw-mb-5 ">
            Proposition
          </h3>
        </div>
      </div>
      <div className="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 md:tw-grid-cols-3 lg:tw-grid-cols-4 tw-gap-6 tw-ml-0">
        {courses.map((course, index) => (
          <div
            key={index}
            className="tw-rounded-2xl tw-shadow-md tw-border tw-border-gray-200 tw-overflow-hidden"
          >
            {/* Thumbnail */}
            <div className="tw-relative">
              <a href="/developments/playback">
                <img
