...
const UploadFailedDialog: React.FC<UploadFailedDialogProps> = ({
  ...
  return (
    <Dialog
      ...
      handler={onClose}
    >
      <DialogHeader className="tw-justify-center tw-text-center tw-w-full">{t('uploadFailedDialog.failedFileUploads')}</DialogHeader>
      <DialogBody className="!tw-p-4 tw-text-center tw-text-blue-gray-900 ">
        <Typography className="tw-mb-4 !tw-text-blue-gray-900 ">{!sameDevice ? t('uploadFailedDialog.infoTextOtherDevice', {deviceType: deviceType}) : t('uploadFailedDialog.infoText')}</Typography>
      </DialogBody>
      <DialogFooter className="tw-flex tw-justify-center tw-gap-4">
        ...
        >
          {isDisabled ? <Spinner className="tw-mx-2" /> : t("uploadFailedDialog.reUpload")}
        </button>}
      </DialogFooter>
    </Dialog>
  );
