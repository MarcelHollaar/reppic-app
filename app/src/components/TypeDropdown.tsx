...
const TypeDropdown = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => {
  ...
  const options =  ["General", "Example", "Exercise"];
      ...
      <div className="tw-relative">
        <div className="tw-flex tw-items-center tw-relative">
          <input
            ...
            placeholder={t("typeDropdown.type")}
            name="type"
            value={value}
            readOnly
        ...
        {isOpen && (
          <div className="tw-absolute tw-w-full tw-mt-1 !tw-z-50 tw-bg-white tw-rounded-3xl tw-shadow-lg tw-border tw-border-gray-200 tw-max-h-60 tw-overflow-y-auto" style={{ borderRadius:  "5px" }}>
            ...
            {options.map((type) => (
              ...
              >
                {t(`typeDropdown.${type.toLowerCase()}`)}
              </div>
            ))}
          </div>
        )}
