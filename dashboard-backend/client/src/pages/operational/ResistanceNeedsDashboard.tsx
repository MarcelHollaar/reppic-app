...
export default function ResistanceNeedsDashboard() {
  ...
  const topResistances = ((analytics?.resistanceNeeds?.resistances || []) as any[]).map((item, i) => ({
    ...item,
    color: chartColors[i % chartColors.length]
  }));

  const commercialTriggers = ((analytics?.resistanceNeeds?.triggers || []) as any[]).map((item, i) => ({
    ...item,
    color: chartColors[i % chartColors.length]
  }));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-semibold mb-1">{t.resistanceNeeds}</h1>
          <p className="text-sm text-muted-foreground">
            {t.resistanceNeedsDesc}
          </p>
        </div>
        <div className="flex gap-3">
          <PdfDownloadButton 
            dashboardTitle={buildOperationalReportTitle(language)}
            dashboardSubtitle={t.resistanceNeedsDesc}
            period={formatSelectedDate(selectedDate, language)}
            conclusion={analytics?.resistanceNeeds?.comparison}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <PieChartCard
          title={t.topResistances}
          data={topResistances}
          alerts={analytics?.resistanceNeeds?.resistanceAlerts}
          summary={analytics?.resistanceNeeds?.resistanceSummary}
        />
        <PieChartCard
          title={t.commercialTriggers}
          data={commercialTriggers}
          alerts={analytics?.resistanceNeeds?.triggerAlerts}
          summary={analytics?.resistanceNeeds?.triggerSummary}
        />
      </div>
